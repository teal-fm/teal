...
- Top albums chart (2026-09-25): [PR #231](https://github.com/teal-fm/teal/pull/231) is stacked on `fix/album-tracklist-and-cover-art`; CAR fix committed as `47c3009` and pushed to GitHub and Tangled. Public preview: `https://zone-frontpage-tracked-homework.trycloudflare.com/profile/matt.evil.gay/topster?period=all&size=3`. The quick tunnel serves a compressed static Amethyst export through Caddy and a local Aqua build with a CAR-backed, 15-minute Redis-cached chart endpoint. CAR audit for `matt.evil.gay`: 105,073 play records, 103,291 with album names, including 31,121 without release MBIDs that the old indexed SQL chart omitted. Public endpoint returned 105,073/103,291 and the chart rendered in Chrome; PNG export completed. Rust check/clippy/test, Amethyst typecheck/export, lexicon validation, and Compose config passed. The quick-tunnel hostname is ephemeral and depends on Caddy, cloudflared, Aqua, local Postgres, and Garnet running. The preview's Caddy config and static export are currently inside the running container and will be lost if it is recreated; restart the export and reload Caddy if needed.
