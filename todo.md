...
- [ ] [#86](https://github.com/teal-fm/teal/issues/86) / [TEAL-31](https://linear.app/tealfm/issue/TEAL-31/log-unique-tracks-and-albums-to-popfeed) Log unique tracks and albums to Popfeed
  ...
  - Partial implementation: Amethyst settings now has a persisted opt-in toggle, manual scrobble submission uses the published `bookhive.buzz` `social.popfeed.feed.list` / `social.popfeed.feed.listItem` lexicons to reuse an existing default Popfeed list or create `Teal listens`, then add first-seen track and album/EP list items with MusicBrainz identifiers. Settings also has a signed-in "Sync recent plays" action that backfills paginated Aqua-indexed plays through the user's PDS session, exactly capped at 1,000 plays per run. Focused tests cover the list, list item mapping, duplicate-skip behavior, batch sync, paginated backfill, exact cap handling, and zero-cap no-op behavior. The product/backend decision is documented in `docs/popfeed-sync-decision.md`: keep Popfeed writes in authenticated Amethyst user flows for now, and do not add Cadet/background Popfeed writes without delegated server-side auth and user-facing sync status. Remaining QA requires an authenticated submit/backfill with the toggle enabled.
