...
- Album cover art fix (2026-09-15): `fm.teal.music.getAlbum` now returns `releaseGroupMbid` from MusicBrainz, and the Amethyst album page prefers Cover Art Archive release-group art (the canonical cover streaming services show), falling back to release art and then a placeholder. Fixes releases whose edition art differs from the album (for example `a5e766b8-650c-40ce-a19f-3dc3c865a3e2`). Verified with lexicon generation/validation, offline Aqua check/Clippy, `repos::music` tests, `pnpm typecheck`, Biome, and the Amethyst web export; live preview QA pending.
