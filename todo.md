...
- [x] Re-ran duplicate cleanup on preview Postgres on 2026-06-13: removed 24,237 logical duplicate `plays` rows and 25,262 `play_to_artists_extended` joins, then refreshed play-count materialized views and verified zero remaining duplicate groups.
...
- [x] Fix `pnpm backfill:lightrail:retry` setup by moving Amethyst's Expo native prebuild out of the package `install` lifecycle and into an explicit `prebuild:native` script, so pnpm install/build-script checks no longer fail Rust-only backfill commands.
- [x] Deployed profile social posts to the stable preview on 2026-06-13: rebuilt and restarted Aqua plus Amethyst for `https://sigilyph.teal.fm`, verified public metadata/latest-play XRPC, verified actor-filtered social post XRPC, and smoke-tested a live profile page showing recent posts above recent plays.
