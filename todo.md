...
- Teal sidebar identity alignment (2026-08-12): the authenticated desktop sidebar now loads the logged-in Teal profile directly from Aqua and never falls back to Bluesky name/avatar metadata, keeping it consistent with Teal-authored feed cards. Verify with authenticated browser QA after the next preview rebuild.
