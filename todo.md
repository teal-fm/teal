...
- [x] Add a periodic refresh job for Cadet's play-count materialized views now that live ingestion defers per-play refreshes. Deployed and verified on 2026-09-05: Cadet logged successful refreshes at 18:03:47, 18:04:45, and 18:05:45 UTC while ingesting live records. Public metadata and latest-listen XRPC checks pass; PostgreSQL regression CI passed for commit `356561f`.
- Session restore fix (2026-09-07): Chrome QA completed the provider authorization and callback for the throwaway account, then found a persisted-status race that redirected protected pages during agent restoration. Persistence now excludes runtime clients and login readiness, including legacy saves. A failing regression reproduced the bug; the fix passes TypeScript and all 235 active Amethyst library tests. Deploying the fix before repeating reload and manual-listen QA.
...
- Album lookup timeout (2026-09-07): MusicBrainz stopped responding during live QA, leaving the form busy until the browser network timeout. Requests now abort after 15 seconds with a retryable error, including stalled response bodies. A regression test verifies timeout and queue recovery; TypeScript and all 236 active Amethyst tests pass. MusicBrainz subsequently returned release search results; track selection and submission QA continue.
