...
- Top albums chart (2026-09-25): [PR #231](https://github.com/teal-fm/teal/pull/231) is stacked on `fix/album-tracklist-and-cover-art`. Amethyst has a profile-linked, shareable Topster-style chart ranked by indexed album plays. It supports 3×3, 4×4, and 5×5 grids, the existing stats periods, linked albums, and browser PNG downloads. The chart uses `fm.teal.stats.getUserTopReleases` and shows an empty or error state when data is unavailable. Browser visual QA of the new route remains to be done.
