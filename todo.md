...
- Cadet has a TAP backfill consumer and `pnpm backfill` command. The command starts TAP in full-network mode with `TAP_COLLECTION_FILTERS=fm.teal.*` and consumes TAP record events through the existing Teal ingestors.
