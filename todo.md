...
- [ ] [#86](https://github.com/teal-fm/teal/issues/86) / [TEAL-31](https://linear.app/tealfm/issue/TEAL-31/log-unique-tracks-and-albums-to-popfeed) Log unique tracks and albums to Popfeed
  ...
  - Partial implementation: Amethyst settings now has a persisted opt-in toggle, manual scrobble submission uses the published `bookhive.buzz` `social.popfeed.feed.list` / `social.popfeed.feed.listItem` lexicons to reuse an existing default Popfeed list or create `Teal listens`, then add first-seen track and album/EP list items with MusicBrainz identifiers. Settings also has a signed-in "Sync recent plays" action that backfills the first page of Aqua-indexed plays through the user's PDS session. Focused tests cover the list, list item mapping, duplicate-skip behavior, and batch sync. Remaining QA requires an authenticated submit/backfill with the toggle enabled, and the product/backend follow-up is deciding whether Cadet or another background path should also sync future non-manual ingestion plus deeper history backfill.
