...
- [ ] Complete ATProto OAuth sign-in and callback QA through `https://sigilyph.teal.fm`.
  - Verified again on 2026-06-15 that `pnpm tunnel:verify` validates the stable-origin `client_id`, callback URI, `client_uri`, DPoP setting, and latest plays XRPC response. Remaining QA requires an interactive ATProto login/callback with a real account session.
