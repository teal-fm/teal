...
/** Response marker for the `fm.teal.stats.getUserTopArtists` query.
...
/** Endpoint marker for the `fm.teal.stats.getUserTopArtists` query.

Path: `/xrpc/fm.teal.stats.getUserTopArtists`. The request payload type is `GetUserTopArtists<S>`; send that request with `jacquard::Client` or use this marker through lower-level `XrpcEndpoint` APIs.*/
