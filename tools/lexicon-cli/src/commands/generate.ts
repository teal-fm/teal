...
export async function generate(options: GenerateOptions = {}) {
  ...
  try {
    ...
    console.error(pc.red('❌ Generation failed:'), error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
}
...
async function generateTypeScript(workspaceRoot: string, force?: boolean) {
  ...
  if (!existsSync(packagesLexiconsPath)) {
    console.log(pc.yellow('    ⚠️  TypeScript lexicons package not found, skipping TypeScript generation'));
    return;
  }
